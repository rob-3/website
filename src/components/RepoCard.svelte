<script>
	const data = (async () => {
		const data = await (await fetch(`https://api.github.com/repos/rob-3/steel-lang`)).json();
		return data;
	})();
	/*
	let data = {
...
{:then data}
	<div class="card">
		...
		<div style="display: {data.fork ? 'block' : 'none'}; font-size: 12px; color: #586069;">
			Forked from <a
				style="color: inherit; text-decoration: none;"
				href={data.fork ? data.source.html_url : ''}>{data.fork ? data.source.full_name : ''}</a
			>
		</div>
		...
		<div style="font-size: 12px; color: #586069; display: flex;">
			<div style="{data.language ? '' : 'display: none'}; margin-right: 16px;">
				...
				{:then}
					<span
						style="width: 12px; height: 12px; border-radius: 100%; background-color: {data.language
			...
			<div
				style="display: {data.stargazers_count == 0
					? 'none'
